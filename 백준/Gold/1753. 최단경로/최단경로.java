...
public class Main{
	...
	public static void main(String[] args) throws IOException {
        ...
        int[] in = new int[inputLine.length];
        for (int i = 0; i < inputLine.length; i++) {
            in[i] = Integer.parseInt(inputLine[i]);
        }
	...
	private static void dijkstra(int s) {
		...
		while (!pq.isEmpty()) {
            Edge now = pq.poll();
            // update dist based on information connected to the point to be searched
            for (int j = 0; j < list[now.i].size(); j++) {
                Edge next = list[now.i].get(j);
                int newDistance = now.v + next.v;
                if (dist[next.i] > newDistance) {
                    dist[next.i] = newDistance;
